function [aadDCT] = PNCC_spectrogram(rawdata, fsamp, win_shift, win_length, freq_range, num_bands)
...
%     ad_x = specsub(rawdata, fsamp);	
	...
	dSampRate   = fsamp;
	dLowFreq      = freq_range(1);
	dHighFreq     = freq_range(2);
	dPowerCoeff = 1 / 15;
	...
	% Flags
       %
	bPreem         = 1;	% pre-emphasis coefficient
	bSSF             = 1;
	bPowerLaw    = 1;
	bDisplay        = 0;
     

	dFrameLen     = win_length/1000;  % 25.6 ms window length, which is the default setting in CMU Sphinx
	dFramePeriod = win_shift/1000;   % 10 ms frame period
	iPowerFactor  = 1;
    ...
    aadDCT = aadSpec';  % Without DCT, normalization and temporal derivative
