function A = adjacency_parfor(DATA, TYPE, PARAM);
...
D(floor(n/step)).I = [];
  parfor temp=1:ceil(n/step)
    ...
    D(temp).Z = Z(:,2:PARAM+1);
    D(temp).I = I(:,2:PARAM+1);
    
    for i=i1:i2
      if ( mod(i, 500) ==0) 
	     disp(sprintf('%d points processed.', i));
      end;
%       D{temp,1} = Z(i-i1+1,2:PARAM+1);
    end
...
disp('L2 Distance computation complete');

for temp=1:ceil(n/step)
    i1 = (temp-1)*step + 1;
    i2 = i1+step-1;
    if (i2> n) 
      i2=n;
...
%         disp(sprintf('%d points processed.', i));
%       end
%         A(D(temp).I,i1:i2)= D(temp).Z;   
%     end;
    for i=i1:i2
      for j=1:PARAM
        A(i,D(temp).I(i-i1+1,j))= D(temp).Z(i-i1+1,j); 
        A(D(temp).I(i-i1+1,j),i)= D(temp).Z(i-i1+1,j); 
      end;    
    end

 disp(sprintf('Iter %d of %d',temp,ceil(n/step)));
